...
- `InheritanceType.SINGLE_TABLE` создаст таблицу в которой будут колонки для всех полей, всех классов иерархии + колонка дискриминатора(по дефолту с именем DTYPE, строкового типа и хранящая имя энтити).
...
- Для переопределения аттрибутов в @Embedded классе, нужно над полем встраимываемого типа вешать аннотацию `@AttributeOverride(s)`.
...
- `LockModeType.OPTIMISTIC` - cревнение версий выполняется только при модификации записи в текущей транзакции. Если мы просто прочитали, потом кто-то в соседней транзакции поменял запись, мы про это не узнаем при простом чтении
