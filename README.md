...
- Для того, чтобы вручную настроить маппинг из SQL в энтитю используют аннотации `@SqlResultSetMapping, @FieldResult (если мапится через @EntittyResult)`;
- Даже если одно поле не мапится по умолчанию, придётся перечислить все столбцы и поля;
- В хиберенейте, маппинг результатов запроса на энтитю можно задать в самом запросе, с помощью символов подстановки с запросами, стр 494; 
- Или, если запрос нельзя модифицировать, можно маппинг добавить с помощью методов `SQLQuery.addRoot` и `SQLQuery.addProperty`;
- Можно переопределить метод используенмый по умолчанию для загрузки данных(используемый в em.find например), для этого нужно 
    - создать _@NamedNativeQuery_;
    - активировать его в аннотации _@Loader_;
#### JPQL
...
- count(distinct i.name) удаляет дубликаты из подсчёта;
